...
contract TokenStorage is ERC721URIStorage{
    ...
    string private ERR_NOT_ON_SIDE_CHAIN = "The art piece must be stored on the sidechain";
    string private ERR_ALREADY_STORED = "There is already an NFT using this name";
    string private ERR_NOT_OWNER = "You are not the owner of the NFT";
    string private ERR_HAS_DEBT = "You must pay for the maintenance cost";
    ...
    */
    constructor() public ERC721("NFTArt", "NFTArt"){
    ...
    */
    function mintNFT(bool on_sidechain, string memory token_name, string memory image_url, address recipient)
        external
    {
        require(on_sidechain, ERR_NOT_ON_SIDE_CHAIN);
   ...
   /*
    *@dev allows a user to pay for the maintenance cost
    */
    function payMaintenance(address to)
        public
        payable
    {
        address payable beneficiary = payable(to);
    ...
    /*
    *@dev removes the token from the platform
    */ 
    function retireToken(string memory token_name, bool has_debt)
        public
    {
        require(ownerOf(getTokenId(token_name)) == msg.sender, ERR_NOT_OWNER);
        require(!has_debt, ERR_HAS_DEBT);
        uint256 token_id = getTokenId(token_name);
        _storage[token_name] = 0;
        removeToken(msg.sender, token_id);
    }
